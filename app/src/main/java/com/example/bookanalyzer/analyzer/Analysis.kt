...
class BookAnalyzer(private val ctx: Context) {
    fun getAnalysis(path: String): AnalyzedBookModel {
        ...
        return AnalyzedBookModel(
            path, uniqueWordCount, wordCount, charCount, avgSentenceLen.first,
    ...
    private fun calcAvgSentenceLen(wordCount: Int, simpleText: String): Pair<Double, Double> {
        ...
        val avgSentenceLenWrd =
            if (sentenceCount != 0) roundDouble(wordCount.toDouble() / sentenceCount) else 0.0
        val avgSentenceLenChr =
            if (wordCount != 0) roundDouble((charCount.toDouble()) / sentences.size) else 0.0
